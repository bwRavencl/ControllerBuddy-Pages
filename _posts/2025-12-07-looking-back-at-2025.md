...
For me, 2025 was somewhat of a turning point with my switch to [Linux](https://www.linux.org/) (I use [Arch](https://archlinux.org/), by the way) as my daily driver.

Having used Linux for gaming and simming on and off during the mid-2000s, I'm by no means a novice to Linux as a desktop operating system. Over the past few years, mainly due to [Valve Software's](https://www.valvesoftware.com/) initiatives, gaming on Linux has become easier than ever. Compatibility and performance have improved by leaps and bounds, reaching the point where Linux is now a viable alternative to Windows, for most flight simulators.
...
ControllerBuddy has supported Linux for quite some time, but a major obstacle was the inability to run the configuration scripts within [Wine](https://www.winehq.org/) / [Proton](https://github.com/ValveSoftware/Proton). This was because they required Windows PowerShell 5.1 to run, and while PowerShell 7+ works great with Wine, there is no feasible way to get older releases working.
...
For handling input devices, ControllerBuddy had relied on [GLFW](https://www.glfw.org/) for several years. This was coupled with a lot of custom code to support advanced features of Sony controllers via a proprietary HID driver. On Windows, [jxinput](https://github.com/StrikerX3/JXInput) was used for native XInput controller support, and on Linux, more custom code was needed to implement what was frankly pretty poor rumble support.

With the release of [SDL3](https://www.libsdl.org/) in January and its support arriving shortly thereafter in [LWJGL](https://www.lwjgl.org/), I saw an opportunity to switch ControllerBuddy to this brand-new backend. This move effectively reduced the amount of code required for handling all these different controller types to zero, while simultaneously improving compatibility.

The only negative consequence of the switch to SDL3 was that I had to strip support for [OpenVR](https://github.com/ValveSoftware/openvr)-based overlays. While this is certainly a shame, I doubt it significantly affected many users. OpenVR has largely been replaced by [OpenXR](https://www.khronos.org/openxr/), and supporting the latter is a completely different challenge.
...
Continuing my quest to reduce dependencies, the next goal was to eliminate the [JNA](https://github.com/java-native-access/jna) library, which allows convenient access to native functions from within Java.

* On Windows, JNA was used by ControllerBuddy primarily to communicate with the [vJoy](http://vjoystick.sourceforge.net/) driver and the Windows API.
* On Linux, JNA was used as part of [linuxio4j](https://github.com/bithatch/linuxio4j) to communicate with the UINPUT module.

While JNA is easy to use, a modern alternative, the [Function and Memory (FFM) API](https://openjdk.org/jeps/442), was finalized with Java 22. It represents the future-proof and most efficient way to access native APIs from within the JVM.  
...
My efforts to remove third-party dependencies also extended to the [Gradle](https://gradle.org/) build script.
...
Additionally, I took this opportunity to port the entire build script from [Groovy](https://groovy-lang.org/) to [Kotlin](https://kotlinlang.org/).
...
At the beginning of this month, I replaced the previously used [Apache Batik](https://xmlgraphics.apache.org/batik/) library with the more modern [jsvg](https://github.com/weisJ/jsvg) alternative.  
jsvg is used to render the [SVG](https://en.wikipedia.org/wiki/SVG) visualizations of profiles.  
While I had to introduce more custom code to handle the correct sizing of the [viewBox](https://developer.mozilla.org/en-US/docs/Web/SVG/Reference/Attribute/viewBox) (i.e. the viewport of an SVG), the switch had the benefit of reducing the overall size of the entire ControllerBuddy folder by about 3 to 4 MB, depending on the platform.
